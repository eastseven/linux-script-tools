...
check_disk()
{
  script -q -c "fdisk -l" $LOCKfile >/dev/null
  disk_list_tmp=$(cat $LOCKfile|grep "doesn't contain a valid partition table"|awk '{print $2}'|grep xv)
  >$LOCKfile
  now_mount=$(df -h)
  for i in `echo $disk_list_tmp`
  do
...
main()
{
  ...
  while read a b
  do
    echo "${a}             $b                 ext3    defaults        0 0" >>/etc/fstab
  done <$LOCKfile
  mount -a
}
